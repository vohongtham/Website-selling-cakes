...
class OrderService {
    ...
    async deleteByCustomerId(mskhId) {
        try {
            console.log(`Deleting orders for customer ID: ${mskhId}`);
            const result = await this.Order.deleteMany({ mskhId });
            console.log(`Deleted ${result.deletedCount} orders.`);
            return result.deletedCount;
        } catch (error) {
            console.error("Error in deleteByCustomerId:", error);
            throw error;
        }
    }
